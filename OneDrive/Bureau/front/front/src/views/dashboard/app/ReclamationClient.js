...
const ReclamationList = ({ role }) => {
  ...
  const currentUserRole = localStorage.getItem('userRole'); 
   ...
   useEffect(() => {
        ...
        if (currentUserRole !== "CLIENT_DISPATCHER") {
  ...
  const handleShowLieu = (fullLieu, lat, lng) => {
    setViewMoreData({
      ...
      lat: lat || 33.8869, // Fallback to default latitude
      lng: lng || 9.5375, // Fallback to default longitude
    });
  };
...
const handleEditClick = (item) => {
  ...
  >
    View more
      ...
      <MapContainer
        center={[currentReclamation.lat || 33.8869, currentReclamation.lng || 9.5375]}
        zoom={13}
        style={{ height: '100%', width: '100%' }}
      >
        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        {currentReclamation.lat !== undefined && currentReclamation.lng !== undefined && (
