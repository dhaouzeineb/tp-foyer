...
// Import des composants de barre latérale
import SidebarClient from './sidebarClient'; // Assurez-vous que ce fichier existe
import SidebarAgent from './sidebarAgent'; // Assurez-vous que ce fichier existe
import SidebarAdmin from './vertical-nav'; // Assurez-vous que ce fichier existe
...
const Sidebar = memo((props) => {
  ...
  useEffect(() => {
    ...
    // Récupération du rôle de l'utilisateur à partir du localStorage
    const role = localStorage.getItem('userRole');
    setUserRole(role); // Mise à jour de l'état du rôle
  }, []);

  const renderSidebar = () => {
    if (userRole === 'CLIENT_DISPATCHER') {
      return <SidebarClient />;
    } else if (userRole === 'AGENT_DISPATCHER') {
      return <SidebarAgent />;
    } else if (userRole === 'ADMIN_DISPATCHER') {
      return <SidebarAdmin />;
    }
    return <VerticalNav />; // Option par défaut si aucun rôle n'est trouvé
  };
      ...
      >
        ...
        >
          ...
          <div className="sidebar-list navbar-collapse" id="sidebar">
            {renderSidebar()} {/* Affichage de la barre latérale selon le rôle */}
          </div>
        </div>
